...
export default class LucidDataTable extends DatabaseDataTable {
  ...
  protected compileQuerySearch(query: ModelQueryBuilderContract<LucidModel, any>, columnName: string, keyword: string, boolean: string = "or"): void {
  ...
  protected isNotEagerLoaded(relation: string) {
    return (
      relation === "" || !Object.keys(Object.fromEntries(this.query.model["$relationsDefinitions"])).includes(relation) || relation === this.query.model.name
    );
  }
