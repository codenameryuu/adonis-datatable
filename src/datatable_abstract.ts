...
export abstract class DataTableAbstract implements DataTable {
  ...
  protected isBlacklisted(column: string): boolean {
    ...
    if (columnDef["whitelist"] === "*" || (columnDef["whitelist"] && columnDef["whitelist"].includes(column))) {
      return false;
    }
  ...
  protected processResults(results: Record<string, any>[]): Record<string, any>[] {
    const processor = new DataProcessor(results, this.getColumnsDefinition(), this.$templates, this.request.start(), this.config);

    return processor.process(this.$dataObject);
  }

  protected render(data: any[]): Record<string, any> {
    let result = this.attachAppends({
    ...
    if (this.config.isDebugging()) {
      result = this.showDebugger(result);
    }
  ...
  protected getColumnNameByIndex(index: number): string {
    const name = this.$columns[index] !== undefined && this.$columns[index] !== "*" ? this.$columns[index] : this.getPrimaryKeyName();
  ...
  getColumnsDefinition(): Record<string, any> {
    ...
    return Helper.objectReplaceRecursive(Helper.objectIntersectKey(datatableColumns, allowed), this.$columnDef);
  }
  ...
  setRowClass(content: (<T extends abstract new (...args: any) => any>(query: InstanceType<T>) => string | number) | string): this {
    this.$templates["DT_RowClass"] = content;

    return this;
  }

  setRowId(content: (<T extends abstract new (...args: any) => any>(query: InstanceType<T>) => string | number) | string): this {
    this.$templates["DT_RowId"] = content;

    return this;
  }

  setRowData(data: Record<string, (<T extends abstract new (...args: any) => any>(query: InstanceType<T>) => string) | string>): this {
    this.$templates["DT_RowData"] = data;

    return this;
  }

  addRowData(key: string, value: (<T extends abstract new (...args: any) => any>(query: InstanceType<T>) => string) | string): this {
    this.$templates["DT_RowData"][key] = value;

    return this;
  }

  setRowAttr(data: Record<string, (<T extends abstract new (...args: any) => any>(query: InstanceType<T>) => string | number) | string | number>): this {
    this.$templates["DT_RowAttr"] = data;

    return this;
  }

  addRowAttr(key: string, value: (<T extends abstract new (...args: any) => any>(query: InstanceType<T>) => string | number) | string): this {
  ...
  withQuery(key: string, callback: <T extends abstract new (...args: any) => any>(query: InstanceType<T>) => string): this {
  ...
  filter(callback: <T extends abstract new (...args: any) => any>(query: InstanceType<T>) => void, globalSearch: boolean = false): this {
